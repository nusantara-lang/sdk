...
std::string Lexer::Token::ubahKeString() const {
    return std::format(
      "{}:{}:{} {} {}", this->sumber, this->baris + 1,
      this->karakter + 1, this->tipe.ubahKeString(),
      (this->nilai == "\n")      ? "\\n"
      : (this->nilai == "\t")    ? "\\t"
      : (this->nilai == "\r")    ? "\\r"
      : (this->nilai[0] == '\0') ? "\\0"
...
const Lexer::TipeToken& Lexer::Token::getTipe() const {
    return this->tipe;
}

const std::string& Lexer::Token::getSumber() const {
    return this->sumber;
}

const size_t& Lexer::Token::getBaris() const {
    return this->baris;
}

const size_t& Lexer::Token::getKarakter() const {
    return this->karakter;
}

const std::string& Lexer::Token::getNilai() const {
    return this->nilai;
}

void Lexer::Token::setTipe(const TipeToken& tipe) {
    this->tipe = tipe;
}
...
void Lexer::Token::setBaris(const size_t& baris) {
    this->baris = baris;
}
...
void Lexer::Token::setNilai(const std::string& nilai) {
    this->nilai = nilai;
}

void Lexer::Token::setNilai(const char& nilai) {
    this->nilai = nilai;
}
