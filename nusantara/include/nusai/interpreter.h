...
namespace Nusai {
    ...
    class Interpreter: private Nusap::Visitor {
        ...
        private:
            ...
            Variabel& buatVariabel(const Nusal::TipeToken& tipe, const std::string& nama, const std::any& nilai);
            Variabel& ambilVariabel(const std::string& nama);

            Fungsi& buatFungsi(const Nusal::TipeToken& tipe, const std::string& nama, const std::function<std::any(std::map<std::string, Variabel>&)>& definisi);
