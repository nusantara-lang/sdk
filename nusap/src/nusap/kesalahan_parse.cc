...
nusap::kesalahan_parser::kesalahan_parser(const nusal::token& token, const std::string& pesan) {
    ...
    ostream << std::format("{}:{}:{}", token.sumber, token.baris.nilai + 1, token.karakter.nilai + 1) << "\n\n";
    std::string prefix = std::format("{}| ", token.baris.nilai + 1);
    ostream << prefix  << kumpulan_konten_perbaris[token.baris.nilai] << "\n";
    ostream << std::string(token.karakter.nilai + prefix.length(), ' ') << std::string(token.nilai.length(), '^') << "\n";
    ostream << pesan << "\n";
