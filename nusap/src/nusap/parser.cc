...
std::unique_ptr<nusap::node> nusap::parser::buat_node_aturan(const nusap::tipe_node& tipe) {
  return std::make_unique<node>(tipe);
}

std::unique_ptr<nusap::node> nusap::parser::buat_node_token() {
  return std::make_unique<node>(tipe_node::token, std::make_unique<nusal::token>(*this->token_saat_ini));
}

std::unique_ptr<nusap::node> nusap::parser::parse() {
  std::unique_ptr<node> nusantara = this->buat_node_aturan(tipe_node::nusantara);
  this->token_selanjut_nya();
  while(this->token_saat_ini != nullptr) {
    this->lewati_spasi_putih();
    if(this->token_saat_ini->tipe == nusal::tipe_token::TIDAK_DIKETAHUI) {
      throw kesalahan_parser(*this->token_saat_ini, std::format(
          "Nusantara tidak dapat mengenali karakter '{}'.", this->token_saat_ini->nilai
      ));
    }
...
std::unique_ptr<nusap::node> nusap::parser::parse_pernyataan() {
  std::unique_ptr<node> pernyataan = this->buat_node_aturan(tipe_node::pernyataan);
  if(this->token_saat_ini_adalah(nusal::tipe_token::IDENTIFIKASI)) {
    pernyataan->children.push_back(this->parse_buat_fungsi());
    return pernyataan;
  }
  if(this->token_saat_ini_atau_adalah({nusal::tipe_token::TANDA_HUBUNG, nusal::tipe_token::ANGKA, nusal::tipe_token::TEKS})) {
...
std::unique_ptr<nusap::node> nusap::parser::parse_pernyataan_blok_kode() {
  ...
  if(this->token_saat_ini_atau_adalah({nusal::tipe_token::BILANGAN, nusal::tipe_token::TEKS})) {
  ...
  throw kesalahan_parser(*this->token_saat_ini, "Pernyataan blok kode tidak valid.");
}
...
std::unique_ptr<nusap::node> nusap::parser::parse_ekspresi() {
  ...
  if(token_saat_ini_atau_adalah({nusal::tipe_token::TANDA_HUBUNG, nusal::tipe_token::ANGKA})) {
...
std::unique_ptr<nusap::node> nusap::parser::parse_teks() {
  ...
  if(token_saat_ini_adalah(nusal::tipe_token::KUTIP_SATU)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Teks harus di awali kutip satu '.");
  }
  while(!token_saat_ini_adalah(nusal::tipe_token::KUTIP_SATU)) {
    if(token_saat_ini_atau_adalah({nusal::tipe_token::BARIS_BARU, nusal::tipe_token::KEMBALI_KE_AWAL_KARAKTER})) {
      throw kesalahan_parser(*this->token_saat_ini, "Tidak boleh ada baris baru secara eksplisit pada teks");
    }
    teks->children.push_back(this->buat_node_token());
    this->token_selanjut_nya();
  }
  if(token_saat_ini_adalah(nusal::tipe_token::KUTIP_SATU)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Teks harus di akhiri kutip satu '.");
  }
  return teks;
}
...
std::unique_ptr<nusap::node> nusap::parser::parse_bilangan() {
  ...
  if(token_saat_ini_adalah(nusal::tipe_token::TITIK)) {
    throw kesalahan_parser(*this->token_saat_ini, "Di indonesia biasa nya bilangan desimal pake koma ',' bukan '.'");
  }
  if(token_saat_ini_adalah(nusal::tipe_token::KOMA)) {
    bilangan->children.push_back(this->buat_node_token());
    this->token_selanjut_nya();
    if(!token_saat_ini_adalah(nusal::tipe_token::ANGKA)) {
      throw kesalahan_parser(*this->token_saat_ini, "Setelah koma ',' seharus nya ada angka lagi.");
    }
    while(token_saat_ini_adalah(nusal::tipe_token::ANGKA)) {
      bilangan->children.push_back(this->buat_node_token());
      this->token_selanjut_nya();
...
std::unique_ptr<nusap::node> nusap::parser::parse_buat_variable() {
  std::unique_ptr<node> buat_variable = this->buat_node_aturan(tipe_node::buat_variable);
  nusal::tipe_token tipe = nusal::tipe_token::TIDAK_DIKETAHUI;
  if(token_saat_ini_atau_adalah({nusal::tipe_token::TEKS, nusal::tipe_token::BILANGAN})) {
  ...
  if(token_saat_ini_adalah(nusal::tipe_token::IDENTIFIKASI)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Nama untuk variable belum ditulis.");
  }
...
std::unique_ptr<nusap::node> nusap::parser::parse_blok_kode() {
  std::unique_ptr<node> blok_kode = this->buat_node_aturan(tipe_node::blok_kode);
  if(token_saat_ini_adalah(nusal::tipe_token::KURUNG_KURAWAL_BUKA)) {
    blok_kode->children.push_back(this->buat_node_token());
    this->token_selanjut_nya();
    this->lewati_spasi_putih();
  }else{
    throw kesalahan_parser(*this->token_saat_ini, "Blok kode harus di awali kurung kurawal buka '{'.");
  }
  ...
  if(token_saat_ini_adalah(nusal::tipe_token::KURUNG_KURAWAL_TUTUP)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Blok kode harus di akhiri kurung kurawal buka '{'.");
  }
  return blok_kode;
}

std::unique_ptr<nusap::node> nusap::parser::parse_buat_fungsi() {
  std::unique_ptr<node> buat_fungsi = this->buat_node_aturan(tipe_node::buat_fungsi);
  if(this->token_saat_ini_adalah(nusal::tipe_token::IDENTIFIKASI)) {
  ...
  if(this->token_saat_ini_adalah(nusal::tipe_token::KURUNG_BUKA)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Seharusnya kurung buka setelah nama fungsi.");
  }
  if(token_saat_ini_atau_adalah({nusal::tipe_token::TEKS, nusal::tipe_token::BILANGAN})) {
    ...
    while (token_saat_ini_adalah(nusal::tipe_token::KOMA)) {
      ...
      if(token_saat_ini_atau_adalah({nusal::tipe_token::TEKS, nusal::tipe_token::BILANGAN})) {
        ...
        throw kesalahan_parser(*this->token_saat_ini, "Seharusnya setelah koma ada variabel lagi.");
      }
    }
  }
  if(this->token_saat_ini_adalah(nusal::tipe_token::KURUNG_TUTUP)) {
    ...
    throw kesalahan_parser(*this->token_saat_ini, "Setelah kurung buka harus nya kurung tutup ')'.");
  }
  if(token_saat_ini_adalah(nusal::tipe_token::KURUNG_KURAWAL_BUKA)) {
...
std::unique_ptr<nusap::node> nusap::parser::parse_manggil_fungsi() {
  ...
  if(token_saat_ini_atau_adalah({nusal::tipe_token::TANDA_HUBUNG, nusal::tipe_token::ANGKA, nusal::tipe_token::KUTIP_SATU})) {
    ...
    while (token_saat_ini_adalah(nusal::tipe_token::KOMA)) {
      ...
      if(token_saat_ini_atau_adalah({nusal::tipe_token::TANDA_HUBUNG, nusal::tipe_token::ANGKA, nusal::tipe_token::KUTIP_SATU})) {
        ...
        throw kesalahan_parser(*this->token_saat_ini, "Seharusnya setelah koma ada ekspresi lagi.");
      }
    }
  }else if(token_saat_ini_atau_adalah({nusal::tipe_token::TEKS, nusal::tipe_token::BILANGAN})) {
