...
namespace nusal {
  ...
  template<typename T> class lexer {
    public:
      explicit lexer(const std::vector<tipe_token_data<T>>& tipe_token_data):
          tipe_token_data(tipe_token_data) {}

      void input(const std::string& input) {
        if(this->sumber_berserta_input.contains(this->sumber_tidak_diketahui()
           )) {
          this->sumber_berserta_input[this->sumber_tidak_diketahui()] =
              this->sumber_berserta_input[this->sumber_tidak_diketahui()] +
              input;
        } else {
    ...
    private:
      ...
      [[nodiscard]] const std::string& sumber_tidak_diketahui() const {
        static std::string output = "tidak diketahui";
        return output;
      };
  };
